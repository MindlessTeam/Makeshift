...
namespace Makeshift
{
	namespace Util
	{
		...
		void FileSystem::exportMesh(Mesh mesh, const std::string& path)
		{
			...
			if (output)
			{
				...
				// Here we are taking advantage of the fact that we know 
				// what data is supposed to be read in what order
				...
				// When reading the file we know exactly how much data 
				// we need to read in, because we told the file
